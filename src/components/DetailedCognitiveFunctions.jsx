...
const DetailedCognitiveFunctions = () => {
  ...
  const axes = createMemo(
    () => cogFunctionsDetailedData?.cognitiveFunctionDetailed ?? []
  );
  ...
  const currentSectionMeta = createMemo(() =>
    ...
    <div class={styles.detailedContainer} id="detailed-cognitive-functions">
      <div class={styles.introSection}>
        ...
        <div class={styles.explanationBox}>
          ...
          <p>
            Explore each axis through concise essays covering positions
            (dominant → inferior), shadow dynamics, and development.
          </p>
          <p>
            Pick a function to see its sections; use ← → to jump between tabs.
          </p>
        </div>
      </div>
      ...
      <div class={styles.mainContent}>
                    ...
                    <For each={axis.functions}>
                      {(func) => {
                        ...
                        return (
                          <button
                            type="button"
                            class={`${styles.functionButton} ${isSelected ? styles.selectedFunction : ""
                              }`}
                            style={{ "background-color": func.color }}
                            aria-pressed={isSelected}
                            onClick={() =>
                              selectFunction({ ...func, axisTitle: axis.title, axisName: axis.axis })
                            }
                          >
                            <span class={styles.functionSymbol}>
                              {func.symbol}
                            </span>
        ...
        <Show when={selectedFunction()} fallback={
          <div class={styles.defaultState}>
            <div class={styles.welcomeCard}>
              <h2>Welcome to Detailed Cognitive Functions</h2>
              <p>
                Select a function from the axes above to explore developmental
                trajectories, shadow patterns, and positional nuances.
              </p>
              <div class={styles.featureHighlights}>
                <div class={styles.highlight}>
                  ...
                  <p>
                    Dominant, auxiliary, tertiary, inferior — clear contrasts.
                  </p>
                </div>
                <div class={styles.highlight}>
                  <span class={styles.highlightIcon}>🌑</span>
          ...
          <div class={styles.functionDetail}>
            ...
            >
              ...
              <div class={styles.essayContent}>
                <For each={renderParagraphs(currentEssayText())}>
                  {(p) => <p>{p}</p>}
                </For>
              </div>
            </div>
